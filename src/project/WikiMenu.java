...
public class WikiMenu {
	...
	public void readNewMenu(String content) {
		...
		try {
			...
			/** Stores the last ID of the parent from each level */
			int[] parentItem = new int[MAX_DEPTH];
			int i = 0;

			/** -1 means no parent item availible. */
			Arrays.fill(parentItem, -1);

			for (line = b.readLine(); line != null; line = b.readLine()) {
				/*
				 * Ignore comments (leading #). Parse the line.
				 */
				if (line.length() > 0 && line.charAt(0) != '#') {
					...
					/* Set the parent item ID and clear the IDs of sub-parents */
					if (me.level > 0)
						parentItem[me.level - 1] = i;
					for (int j = me.level; j < MAX_DEPTH; j++)
						parentItem[j] = -1;
					 ...
					 * Read settings
					 */
					if (line.contains(Constants.MenuTags.linkDeactivate))
						link_deactivate = true;
					if (line.contains(Constants.MenuTags.linkDeactivateNot))
						link_deactivate = false;
					if (line.contains(Constants.MenuTags.linkStrong))
						link_strong  = true;
					if (line.contains(Constants.MenuTags.linkStrongNot))
						link_strong = false;
				}
			}
	...
	public StringBuffer getMenuAsList(String searchText, SearchLocation where) {
		...
		for (MenuEntry me : menu) {
			...
			/*
			 * Check for open children in the group
			 */
			openChild = false;
			for (MenuEntry mm : getGroup(me))
				for (MenuEntry m : mm.children)
					if (m.isOpen()) {
						openChild = true;
						break;
					}
			 ...
			 */
			s.append(me.getLine(
						 checkOpen(getGroup(me)),
						 (me.parent != null ? me.parent.isOpen() : false),
						 openChild, link_deactivate, link_strong
					 )
					 + "\n");
		}
	...
	private boolean openItem(String text, SearchLocation where) {
		...
		/*
		 * Open all matching entries
		 */
		if (v.size() > 0) {
			for (short s = 0; s < v.size(); s++)
				menu.get(v.get(s)).open();
			return true;
		}
	...
	private Vector<MenuEntry> getGroup(MenuEntry me) {
		...
		if (v2.size() > 0) {
			/**
			 * Add the MEs of the same group (inclusive «me» itself).
			 * They have to have the same level and may not have an item
			 * with a lower level separating two entries.
			 */
			for (short s = v2.get(0); s >= 0; s--) {
				if (menu.get(s).level == me.level)
					v.add(menu.get(s));
				else if (menu.get(s).level < me.level)
					break;
			}

			/**
			 * Add the remaining MEs
			 */
			for (int i = v2.get(0) + 1; i < menu.size(); i++) {
				if (menu.get(i).level == me.level)
					v.add(menu.get(i));
				else if (menu.get(i).level < me.level)
					break;
			}
		}
	...
	private ArrayList<String> getCsvMenu(String s) {
		...
		if (m.find()) {
			...
			if (end < s.length())
				al.add(s.substring(start , s.length() ).replaceAll("\\\\,", ","));
	...
	private Vector<Short> findByLink(String link) {
		...
		for (short i = 0; i < menu.size(); i++)
			if (menu.get(i).link.equals(link))
				v.add(i);
		return v;
	}
		...
		try {
			...
			while ((line = b.readLine()) != null) {
				if (!line.startsWith("*")) {
					...
					out.append((arguments.length() > 0 ? "<h3 " + arguments.trim() + ">": "<h3>")
							   + line.trim()
							   + "</h3>\n");
				}
				else
	...
	private class MenuEntry {
		...
		public void readLine(ArrayList<String> al) {
			try {
				...
				//TODO 0 Doc: arguments for menu file
		...
		public StringBuffer getLine(boolean openGroup, boolean openParent, boolean openChild, boolean link_deactivate, boolean link_strong) {
			...
			if (name.startsWith("----"))
				...
				/* Append the «ul» arguments */
				line.append("((class=\"" + getFirstArgs(openGroup, openParent, openChild, level) + "\")) ");

				/* Append the «li» arguments */
				line.append(getStyleArgs(args) + " | ");

				if (link.length() > 0)
					line.append(
						(link_strong && open ? "<strong>" : "") +
						(link_deactivate && open ? "" :
						 "<a href=\"" +
						 link +
						 "\">") +
						formatEntry(name) +
						(link_deactivate && open ? "" : "</a>") +
						(link_strong && open ? "</strong>" : "")
					);
				else
					line.append(
		...
		private String getStyleArgs(String args) {
			...
			if (isOpen())
				classesList += "open ";
			if (openChild())
				classesList += "openchild ";
			if (openParent())
				classesList += "openparent ";
			if (hasChild())
				classesList += "hasChild ";
			if (hasChild() && !isOpen() && !openParent() && !openChild())
				// TODO 0 Doc: closable menus
				classesList += "notOpen ";

			classesList = (classesList + "level" + level).trim();

			// If other classes have been defined in the menu arguments: Add them
