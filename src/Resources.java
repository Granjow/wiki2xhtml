...
public class Resources {
	...
	static String olClose = "</ol>\n", olOpen = "<ol>";
	static String olOpenHalf = "<ol";
	static String listClose = "</li>", listOpen = " <li>";
	static String listOpen1 = "<li", open2 = ">";

	static String dlClose = "</dl>", dlOpen = "<dl>";
	static String dlOpenHalf = "<dl";
	static String dtClose = "</dt>", dtOpen = "<dt>";
	static String dtOpen1 = "<dt";
	...
	public static String openItem(char c, int indent, String argument) {
		...
		switch (c) {
		case '*':
			return new String(lineSep + sIndent + listOpen1 + argument + open2);
		case '#':
			return new String(lineSep + sIndent + listOpen1 + argument + open2);
		case ':':
			return new String(lineSep + sIndent + ddOpen1 + argument + open2);
		case ';':
			return new String(lineSep + sIndent + dtOpen1 + argument + open2);
	...
	public static String openList(char c, int indent, String argument) {
		...
		switch (c) {
		case '*':
			return new String(lineSep + sIndent + ulOpenHalf + argument + '>');
		case '#':
			return new String(lineSep + sIndent + olOpenHalf + argument + '>');
		case ':':
		case ';':
			return new String(lineSep + sIndent + dlOpenHalf + argument + '>');
	...
	public static String closeList(char c, int indent) {
		...
		switch (c) {
		case '*':
			return new String(lineSep + sIndent + ulClose);
		case '#':
			return new String(lineSep + sIndent + olClose);
		case ':':
			return new String(lineSep + dlClose);
		case ';':
			return new String(lineSep + sIndent + dlClose );
	...
	public static String closeItem(char c, int indent) {
		...
		switch (c) {
		case '*':
			return new String(sIndent + listClose);
		case '#':
			return new String(sIndent + listClose);
		case ':':
			return new String(sIndent + ddClose);
		case ';':
			return new String(sIndent + dtClose);
...
//					"(?<=(?:[" + specialCharacters + "]|^))" +	// Lookbehind: Extremely slow!
		 ...
		 * Pattern: [head([st]*)?]
		 */
		public static final Pattern reckHead = Pattern.compile("\\[head(?:\\|([st]*))?\\]");

		public static final Pattern toc = Pattern.compile("\\{\\{TOC(?:\\|([^\\{\\}\\|]*))?\\}\\}");
	...
	public static final class XmlNames {
		...
		public static final String digit = "[\u0030-\u0039] | [\u0660-\u0669] | [\u06F0-\u06F9] | [\u0966-\u096F] | [\u09E6-\u09EF] | [\u0A66-\u0A6F] | [\u0AE6-\u0AEF] | [\u0B66-\u0B6F] | [\u0BE7-\u0BEF] | [\u0C66-\u0C6F] | [\u0CE6-\u0CEF] | [\u0D66-\u0D6F] | [\u0E50-\u0E59] | [\u0ED0-\u0ED9] | [\u0F20-\u0F29] ";
		...
		public static final String extender = " [\u00B7 \u02D0 \u02D1 \u0387 \u0640 \u0E46 \u0EC6 \u3005] | [\u3031-\u3035] | [\u309D-\u309E] | [\u30FC-\u30FE]";

		public static final String nameChar = "[-._:] | " + letter + '|' + digit + combiningChar + '|' + extender;

		public static final String nameFirstChar = letter + " | _ | : ";

		/** Pattern for XML names; e.g. used for IDs (See section 3.3.1). */
		public static final String namePattern = "(?x)(" + letter + " [ _ : ] )(" + nameChar + ")*";

		public static final Pattern patternNameChar = Pattern.compile("(?x)(" + Resources.XmlNames.nameChar + ")+");
